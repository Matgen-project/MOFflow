    ...
    try:
        ...
        if check_result is not True:
            ...
            try:
                ...
                value = re.findall(pat,open(output_file).read())
                if len(value) != 0:
                    hvf_value = value[-1]
                else:
                    hvf_value = None    
            except:
                print("The outputfile has no hvf value")
        ...
        if not os.path.exists(wrong_calc_filepath):
            os.makedirs(wrong_calc_filepath)
        print("wrong calculation dir created,moving cif file..")
        cifname = filepath.split("/")[-1] + ".cif"
        srcfile = filepath + "/" + cifname
        #print(srcfile)
        desfile = wrong_calc_filepath + cifname
...
if __name__ == '__main__':
    ...
    for mof_name in workpath_list:
        ...
        print(mof_dir)
        hvf_value = read_output(mof_dir) 
        if hvf_value is not None:
            print("Writing calculation results....  " + mof_name )
            with open("./helium_void_fraction_result.txt","a+") as f1:
                f1.writelines(str(mof_name) +" "*3 + format(str(hvf_value),">s")+ "\n")
        else:
